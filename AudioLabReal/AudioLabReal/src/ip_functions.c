...
void play_stream(u32* audio_storage_left, u32* audio_storage_right, int ptr_location){
...
void record_stream(u32* audio_storage_left, u32* audio_storage_right, int max_size, int ptr_location){
	...
	while (!XUartPs_IsReceiveData(UART_BASEADDR)){
			...
			audio_storage_left[ptr_location] = in_left;
			audio_storage_right[ptr_location] = in_right;
			usleep(27);
			ptr_location++;
			if (ptr_location > max_size){
				xil_printf("Playing Audio\r\n");
				play_stream(audio_storage_left, audio_storage_right, ptr_location);
			}
	}
	if (XUartPs_ReadReg(UART_BASEADDR, XUARTPS_FIFO_OFFSET) == 'p'){
		xil_printf("Playing Audio\r\n");
		play_stream(audio_storage_left, audio_storage_right, ptr_location);
	}
	if(XUartPs_ReadReg(UART_BASEADDR, XUARTPS_FIFO_OFFSET) == 'q') menu();
		else record_stream(audio_storage_left, audio_storage_right, max_size, ptr_location);
}
